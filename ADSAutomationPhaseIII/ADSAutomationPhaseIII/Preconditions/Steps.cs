...
namespace ADSAutomationPhaseIII.Preconditions
{
	public static class Steps
	{
		...
		public static void CloseTab(string tabName)
		{
			try
			{
				if(repo.QueryAnalyzer.TabInfo.Exists(new Duration(10000)))
				{
					Ranorex.TabPage tabPage = repo.QueryAnalyzer.Tab;
					tabPage.RightClick();
					
					if(repo.SunAwtWindow.CloseAllTabsInfo.Exists(new Duration(3000)))
					{
		...
		public static void ConnectServer()
		{
			try
			{
				...
				// Check the server connection and throw error it not connected
				// this repoistory will check for database/keyspace/schema under the server[Duration : 30 Sec], if not it will return false
				if(repo.Connection.ServerConnectionInfo.Exists(new Duration(30000)) == false)
				{
		...
		public static void QueryAnalyser()
		{
			try
			{
				...
				try
				{
					repo.QueryAnalyzer.TabInfo.WaitForItemExists(2000);
				}
				catch
				{
					...
					{
						RetryCount = 1;
						throw new Exception("QueryAnalyser failed after Retries");
					}
				}
			}
			catch (Exception ex)
		...
		public static void ClickQAOpenFile()
		{
			try
			{
				...
				try
				{
					repo.Open.FilePathTextboxInfo.WaitForExists(new Duration(2000));
				}
				catch
				{
					if(RetryCount <= Retries)
					...
					{
						RetryCount = 1;
						throw new Exception("ClickQAOpenFile failed after Retries");
					}
				}
				Sleep();
			}
