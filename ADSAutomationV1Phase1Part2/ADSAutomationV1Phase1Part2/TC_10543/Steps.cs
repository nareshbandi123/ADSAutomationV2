...
using ADSAutomationV1Phase1Part2.TC_10542;
using ADSAutomationV1Phase1Part2.TC_10543;
using Ranorex;
using Ranorex.Core;
...
namespace ADSAutomationPhaseII.TC_10543
{
	...
	public static class Steps
	{
		...
		public static void ScanForTables()
		{
			try
			{
				repo.Options.OptionsTableInfo.WaitForItemExists(30000);
		...
		public static void ScanForTablesAgain()
		{
			try
			{
				repo.Options.OptionTableAgainInfo.WaitForItemExists(20000);
				Table table = repo.Options.OptionTableAgain;
				foreach (var row in table.Rows)
				{
					if(row.Cells[0].Text == "Parameterized Script" && row.Cells[1].Text == "true")
					{
						row.Cells[1].ClickThis();
						break;
					}
				}
				Reports.ReportLog("ScanForTablesAgain", Reports.ADSReportLevel.Success, null, Config.TestCaseName);
			}
			catch (Exception e)
			{
				throw new Exception("ScanForTablesAgain " +e.Message);
			}
		}
