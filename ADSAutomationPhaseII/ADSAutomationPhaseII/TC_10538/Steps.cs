...
namespace ADSAutomationPhaseII.TC_10538
{
    public class Steps
    {
		...
		public static void ClickOnConnectionMonitorTab()
        {
    		try
    		{
    			repo.EditServerProperties.ConnectionMonitorInfo.WaitForExists(new Duration(500));
    			repo.EditServerProperties.ConnectionMonitor.ClickThis(); 
		...
		public static void SelectPingIdleConnection()
        {
    		try
    		{
    			repo.EditServerProperties.PingIdleConnection.ClickThis(); 
		...
		public static void ClickOnYes()
        {
    		try
    		{
    			//repo.EditServerProperties.SelfInfo.WaitForExists(new Duration(500));
				//repo.EditServerProperties.YesButton.ClickThis();
				repo.EditServerProperties.SelfInfo.WaitForExists(new Duration(500));
				repo.EditServerProperties.YesButton.Click("47;14");
		...
		public static void ClickOnSave()
        {
    		try
    		{
    			repo.EditServerProperties.SaveInfo.WaitForExists(new Duration(200));
		...
		public static void ClickOnConectionMonitor()
        {
    		try
    		{
    			repo.DataStudio.ConnectionMonitorInfo.WaitForExists(new Duration(500));
		...
		public static void ValidatePing()
		{
			...
			{
				//Ping window is visible
			}
			else
			{
				//ping window is not visble
			}
		}
		...
		public static void CloseConnectionMonitor()
		{
			try
			{
				repo.ConnectionMonitor.CloseInfo.WaitForExists(new Duration(2000));
		...
		public static void ClickOnAutomate()
		{
			try
    		{
    			repo.Application.AutomateInfo.WaitForExists(new Duration(500));
		...
		public static void FormatStatement()
		{
			try
			{
				...
				Preconditions.Steps.Sleep();
				repo.BeautifyOptions.OkButtonInfo.WaitForExists(new Duration(1000));
		...
		{	
			try
			{
				repo.UntitledApplication.QATextEditorInfo.WaitForExists(new Duration(2000));
